  ...
  <menu action='HelpMenu'>
    ...
    def show_path_info(self, w):
        if not self.g_path_info_dlg:
            ...
            self.g_path_info_dlg.vbox.pack_start(sc, True, True)
            #
            ...
            sc.set_size_request(min(w + gu.hig_dlg_vbox.border_width * 2, gtk.gdk.screen_width()*0.9), h)
    ...
    def on_key_press_event(self, widget, event):
        ...
        if (event.type == gtk.gdk.KEY_PRESS
            and event.state == gtk.gdk.MOD1_MASK # Alt key
            and event.keyval in (gtk.keysyms.KP_Left, gtk.keysyms.Left)
            and self.m_history
