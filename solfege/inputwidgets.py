...
class IntervalButtonsWidgetBase(gtk.VBox, cfg.ConfigUtils):
    ...
    def mk_button(self, nr):
        ...
        # the buttons
        self.m_buttons[nr] = b = gtk.Button(
            mpd.Interval.new_from_int(nr).get_name())
    ...
    def __init__(self, exname, callback, sensicallback, vars_to_watch):
        ...
        for y in range(6):
            for x in range(4):
                ...
                self.g_table.attach(b, x, x + 1, y, y + 1)
                i += 1
...
class IntervalButtonsWidget2(IntervalButtonsWidgetBase):
    ...
    def row(self, data):
        ...
        for i in data:
            hbox.pack_start(self.mk_button(i))
